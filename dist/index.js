...
const clearconsole = () => {
    if (!DEVMODE)
        console.clear();
    if (!DEVMODE)
        console.log(`Connected To ${PLUGINNAME()}`);
...
process.on('uncaughtException', (e) => {
    if (DEVMODE)
        console.log("Berp Tried To Crash But Error Was Caught");
});
class DiscordmodPlugin {
    ...
    onEnabled() {
        ...
        this.api.getEventManager().on('PlayerJoin', async (userJoin) => {
            ...
            new Authflow('', `${this.api.path}\\auth`, { relyingParty: 'http://xboxlive.com' }).getXboxToken().then(async (t) => {
                const MicrosoftGT = (await axios_1.default.get(`https://profile.xboxlive.com/users/xuid(${userJoin.getXuid()})/profile/settings?settings=Gamertag`, {
                    headers: {
                ...
                const PFP = (await axios_1.default.get(`https://profile.xboxlive.com/users/xuid(${userJoin.getXuid()})/profile/settings?settings=GameDisplayPicRaw`, {
                    headers: {
                        'x-xbl-contract-version': '2',
                        'Authorization': `XBL3.0 x=${t.userHash};${t.XSTSToken}`,
                        "Accept-Language": "en-US"
                ...
                if (BANNED.includes(userJoin.getDevice())) {
                    ...
                    client.channels.fetch(PLAYERLOGS).then(async (channel) => await channel.send({ embeds: [BKMSG] })).catch();
                }
                else {
                    if (DEVMODE)
                        this.api.getLogger().info(`**${userJoin.getName()} Joined on ${userJoin.getDevice()}**`);
                    ...
                    client.channels.fetch(PLAYERLOGS).then(async (channel) => await channel.send({ embeds: [JoinMSG] })).catch();
                }
            });
        });
        ...
        this.api.getEventManager().on(`PlayerLeft`, async (userLeft) => {
            if (!DISCORD)
                return;
            new Authflow('', `${this.api.path}\\auth`, { relyingParty: 'http://xboxlive.com' }).getXboxToken().then(async (t) => {
                const MicrosoftGT = (await axios_1.default.get(`https://profile.xboxlive.com/users/xuid(${userLeft.getXuid()})/profile/settings?settings=Gamertag`, {
                    headers: {
                        'x-xbl-contract-version': '2',
                        'Authorization': `XBL3.0 x=${t.userHash};${t.XSTSToken}`,
                        "Accept-Language": "en-US"
                ...
                new Authflow('', `${this.api.path}\\auth`, { relyingParty: 'http://xboxlive.com' }).getXboxToken().then(async (t) => {
                    const PFP = (await axios_1.default.get(`https://profile.xboxlive.com/users/xuid(${userLeft.getXuid()})/profile/settings?settings=GameDisplayPicRaw`, {
                        headers: {
                            'x-xbl-contract-version': '2',
                            'Authorization': `XBL3.0 x=${t.userHash};${t.XSTSToken}`,
                            "Accept-Language": "en-US"
                    ...
                    if ((userLeft.getDevice() == "Windows")) {
                        ...
                        if (DEVMODE)
                            this.api.getLogger().info(`**${userLeft.getName()} Left the Realm**`);
                        ...
                        client.channels.fetch(PLAYERLOGS).then(async (channel) => await channel.send({ embeds: [LeftMSG] })).catch();
                    }
                });
            });
        });
        this.api.getEventManager().on('PlayerMessage', async (p) => {
            if (!ANTISPAM)
                return;
            if (this.cooldown.has(`${p.sender.getName()}-${JSON.stringify(p.message)}-${this.cooldown.size}`) && this.cooldown.size == 8)
                this.kickplayer(p.sender, `§8\n§5Realm: ⊳ ${this.api.getConnection().realm.name} ⊲\n§eUser: \`${p.sender.getName()}\`\n\n§cKicked For Spamming Chat`, `Spamming Chat`);
        ...
        this.api.getEventManager().on(`PlayerMessage`, async (userMessage) => {
            if (!DISCORD)
                return;
            new Authflow('', `${this.api.path}\\auth`, { relyingParty: 'http://xboxlive.com' }).getXboxToken().then(async (t) => {
                const PFP = (await axios_1.default.get(`https://profile.xboxlive.com/users/xuid(${userMessage.sender.getXuid()})/profile/settings?settings=GameDisplayPicRaw`, {
                    headers: {
                        'x-xbl-contract-version': '2',
                        'Authorization': `XBL3.0 x=${t.userHash};${t.XSTSToken}`,
                        "Accept-Language": "en-US"
                ...
                if (playername.startsWith("a"))
                    (playermsg.setColor(`#ff5900`));
                if (playername.startsWith("b"))
                    (playermsg.setColor(`#ff5900`));
                if (playername.startsWith("c"))
                    (playermsg.setColor(`#ff5900`));
                if (playername.startsWith("d"))
                    (playermsg.setColor(`#ffb500`));
                if (playername.startsWith("e"))
                    (playermsg.setColor(`#ffb500`));
                if (playername.startsWith("f"))
                    (playermsg.setColor(`#ffb500`));
                if (playername.startsWith("g"))
                    (playermsg.setColor(`#21ff00`));
                if (playername.startsWith("h"))
                    (playermsg.setColor(`#21ff00`));
                if (playername.startsWith("i"))
                    (playermsg.setColor(`#21ff00`));
                if (playername.startsWith("j"))
                    (playermsg.setColor(`#00ffd4`));
                if (playername.startsWith("k"))
                    (playermsg.setColor(`#00ffd4`));
                if (playername.startsWith("l"))
                    (playermsg.setColor(`#00ffd4`));
                if (playername.startsWith("m"))
                    (playermsg.setColor(`#0054ff`));
                if (playername.startsWith("n"))
                    (playermsg.setColor(`#0054ff`));
                if (playername.startsWith("o"))
                    (playermsg.setColor(`#0054ff`));
                if (playername.startsWith("p"))
                    (playermsg.setColor(`#ab00ff`));
                if (playername.startsWith("q"))
                    (playermsg.setColor(`#ab00ff`));
                if (playername.startsWith("r"))
                    (playermsg.setColor(`#ab00ff`));
                if (playername.startsWith("s"))
                    (playermsg.setColor(`#ff00d9`));
                if (playername.startsWith("t"))
                    (playermsg.setColor(`#ff00d9`));
                if (playername.startsWith("u"))
                    (playermsg.setColor(`#ff00d9`));
                if (playername.startsWith("v"))
                    (playermsg.setColor(`#60ff7e`));
                if (playername.startsWith("w"))
                    (playermsg.setColor(`#60ff7e`));
                if (playername.startsWith("x"))
                    (playermsg.setColor(`#60ff7e`));
                if (playername.startsWith("y"))
                    (playermsg.setColor(`#6b60ff`));
                if (playername.startsWith("z"))
                    (playermsg.setColor(`#6b60ff`));
                setTimeout(() => { client.channels.fetch(REALMCHATID).then(async (channel) => await channel.send({ embeds: [playermsg] })).catch(); }, 200);
            });
        });
        if (SHOWDEATHMSGS)
        ...
        if (SHOWSKINCHANGES)
            this.api.getConnection().on('player_skin', async (ps) => {
                ...
                client.channels.fetch(REALMCHATID).then(async (channel) => await channel.send({ embeds: [changeskin] })).catch();
            });
            ...
            this.api.getCommandManager().executeCommand('list', async (res) => {
                ...
                if (commandNameSub === 'kick') {
                    if (!interaction.memberPermissions.has('ADMINISTRATOR'))
                        return interaction.reply(`You don't have permission to run this!`);
                    try {
                ...
                if (commandNameSub === 'list') {
                    ...
                    new Authflow('', `.\\auth`, { relyingParty: 'http://xboxlive.com' }).getXboxToken().then(async (t) => {
                        const { data } = await (0, axios_1.default)(`https://profile.xboxlive.com/users/gt(${interaction.options.getString(`gamertag`)})/profile/settings?settings=Gamertag`, {
                        ...
                        fs.readFile(`./plugins/${PLUGINNAME}/whitelist.json`, 'utf8', (err, data) => {
                            var obj = JSON.parse(data);
                            if (obj.includes(xuid))
                                return;
                            obj.push(xuid);
                            var json = JSON.stringify(obj);
                            fs.writeFile(`./plugins/${PLUGINNAME}/whitelist.json`, json, (err) => {
                                if (err) {
                                    if (DEVMODE)
                                        console.log(err);
                                    return;
                    ...
                    new Authflow('', `.\\auth`, { relyingParty: 'http://xboxlive.com' }).getXboxToken().then(async (t) => {
                        ...
                        fs.readFile(`./plugins/${PLUGINNAME}/whitelist.json`, 'utf8', (err, data) => {
                            if (err)
                                return interaction.reply("Could not read whitelist list! An unexpected error occurred! Try again.");
                            var obj = JSON.parse(data);
                            if (!obj.includes(xuid))
                                return interaction.reply(`User isn't whitelisted yet!`);
    ...
    automod(p) {
        if (!MOD)
            return;
        new Authflow('', `${this.api.path}\\auth`, { relyingParty: 'http://xboxlive.com' }).getXboxToken().then(async (t) => {
            const GamerScore = (await axios_1.default.get(`https://profile.xboxlive.com/users/xuid(${p.getXuid()})/profile/settings?settings=Gamerscore`, {
                headers: {
                    'x-xbl-contract-version': '2',
                    'Authorization': `XBL3.0 x=${t.userHash};${t.XSTSToken}`,
                    "Accept-Language": "en-US"
            ...
            fs.readFile(path_1.default.resolve(`./plugins/${PLUGINNAME}/whitelist.json`), 'utf8', async (err, data) => {
                if (DEVMODE)
                    if (!data || err)
                ...
                new Authflow('', `${this.api.path}\\auth`, { relyingParty: 'http://xboxlive.com' }).getXboxToken().then((t) => {
                    axios_1.default.get(`https://titlehub.xboxlive.com/users/xuid(${p.getXuid()})/titles/titlehistory/decoration/scid,image,detail`, {
                        ...
                        if (GamerScore < GAMERSCORE_MINIMUM && !WHITELIST_PSN_XBOX)
                        ...
                        if (BANNED.includes(res.data.titles[1].name.replace(new RegExp('Minecraft for ', 'g'), '')))
                            return this.kickplayer(p, `§8\n§5Realm: §r⊳ §b${this.api.getConnection().realm.name} §r⊲\n§eUser: ${p.getName()}\n\n§cRecently Played On A Blocked Device!`, `Recently Played On ${res.data.titles[1].name}`);
                        if (BANNED.includes(res.data.titles[2].name.replace(new RegExp('Minecraft for ', 'g'), '')))
    ...
    kickplayer(p, r, rl) {
        ...
        const AUTOMODLOG = new MessageEmbed()
            ...
            .setThumbnail(``);
        setTimeout(() => {
            client.channels.fetch(AUTOMODLOGS).then(async (channel) => await channel.send({ embeds: [AUTOMODLOG] })).catch();
        }, 10000);
    }
    onDisabled() {
        if (DEVMODE)
    ...
    Xmessage(p, m) {
        if (!XBOXMESSAGE)
            return;
        new Authflow('', `${this.api.path}\\auth`, { relyingParty: 'http://xboxlive.com' }).getXboxToken().then(async (t) => {
            const message = await (0, axios_1.default)(`https://xblmessaging.xboxlive.com/network/xbox/users/me/conversations/users/xuid(${p.getXuid()})`, {
                method: 'POST',
                data: {
                    parts: [
                        {
...
module.exports = DiscordmodPlugin;
